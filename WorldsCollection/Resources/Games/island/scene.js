...
function makeLighthouse() {
  ...
  // Bed inside keeper's cottage — the sleep trigger point
  const bedFrame = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.35, 2.2),
    new THREE.MeshLambertMaterial({ color: 0x6b3a1f }));
  bedFrame.position.set(-3.8, 0.37, -0.6);
  group.add(bedFrame);
  const bedMattress = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.22, 2.0),
    new THREE.MeshLambertMaterial({ color: 0xf0e8d8 }));
  bedMattress.position.set(-3.8, 0.66, -0.6);
  group.add(bedMattress);
  const bedPillow = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.14, 0.4),
    new THREE.MeshLambertMaterial({ color: 0xffffff }));
  bedPillow.position.set(-3.8, 0.80, -1.4);
  group.add(bedPillow);
...
export function buildScene(scene) {
  ...
  const seaMat = createSeaMaterial(); // visually tuned zones — sandy bay turquoise, kelp cove green, harbour murky
